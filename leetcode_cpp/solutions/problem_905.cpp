...
template <typename T>
static constexpr std::vector<T> sortArrayByParity(const std::vector<T> &A)
{
    std::vector<T> result;
...
template <typename T>
static constexpr std::vector<T> sortArrayByParity_stdStablePartition(std::vector<T> &result)
{
...
TEST_CASE("problem 905 -- long input")
{
    ...
    std::vector<int> expected = {290, 2984, 2212, 874, 2358, 2846, 1786, 1254, 62, 3708, 228, 1404, 1200, 4766, 1796, 3106, 3578, 1940, 2072, 3254, 1672, 2072, 3363, 4833, 3381, 4227, 1711, 1253, 2049, 2543, 1557, 4189, 2803, 1679, 1761, 1439, 4735, 3169, 7, 961, 1197, 3187, 1893, 4377, 2841, 2011, 3509, 2091, 3311, 233};
    CHECK(expected == sortArrayByParity(input));
}
